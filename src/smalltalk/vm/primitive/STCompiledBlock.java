...
public class STCompiledBlock {
	...
	public STCompiledBlock(STBlock blk) {
		...
		this.nargs=blk.nargs();
		this.nlocals=blk.nlocals();
		...
		if(blk.isMethod()){
			List<Scope> stBlocks = blk.getAllNestedScopes();
			blocks = new STCompiledBlock[stBlocks.size()];
			int blockIndex;
			for(int i=0; i<stBlocks.size();i++){
				blockIndex = ((STBlock)stBlocks.get(i)).index;
				blocks[blockIndex]=((STBlock)stBlocks.get(i)).compiledBlock;
			}
		}

	}

	public void initialLiteralAsStrings(){
		literalsAsSTStrings = new STString[literals.length];
	}
...
//					 templateString==testStringTemplate ?
			  templateString.equals(testStringTemplate) ?
